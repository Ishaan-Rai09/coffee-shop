...
const createSampleUser = async (): Promise<IUser> => {
  try {
    // Check if admin user already exists
    const existingUser = await User.findOne({ email: 'admin@example.com' }) as IUser;
...
const createTestUser = async (): Promise<IUser> => {
  try {
    ...
    const existingUser = await User.findOne({ email: 'user@example.com' }) as IUser;
...
const createSampleOrders = async (): Promise<void> => {
  try {
...
const createSampleBlogPosts = async (userId: mongoose.Types.ObjectId | string) => {
...
const seedDatabase = async () => {
  try {
    ...
    const blogPostsData = await createSampleBlogPosts(user._id);
